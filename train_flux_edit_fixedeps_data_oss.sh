#!/usr/bin/env bash
# EditFlow FLUX.1-Kontext residual PIID, non-split, no alpha, no GAN.
...
#
# Init: FLUX.1-Kontext-dev transformer + randomly initialized delta/mixture heads.
# Data: 30% oss / 70% pico.
#
#   bash train_flux_edit_fixedeps_data_oss.sh
#   FRESH=1 bash train_flux_edit_fixedeps_data_oss.sh
#   RESUME_RUN_DIR=20260902_120000 RESUME_ITER=10000 bash train_flux_edit_fixedeps_data_oss.sh
#   PRETRAIN_CKPT=checkpoints/.../iter_20000.pth FRESH=1 bash train_flux_edit_fixedeps_data_oss.sh
...
RUN_NAME="gmkontext_uedit_fixedeps_k16_${NFE}nfe_shift${SHIFT}_oss30_pico70"
CONFIG="${PROJECT_DIR}/configs/kontext/editflux_uedit_fixedeps_2nfe_k16_data_oss_pico.py"
...
if [[ -z "${RESUME_FROM}" && -n "${PRETRAIN_CKPT}" ]]; then
    ...
    if [[ -e "${_pretrain_path}" ]]; then
        ...
        echo "[pretrain] loading Kontext residual student from ${LOAD_FROM}"
    else
        echo "[pretrain] PRETRAIN_CKPT not found (${PRETRAIN_CKPT})" >&2
        exit 1
    fi
elif [[ -z "${RESUME_FROM}" ]]; then
    echo "[pretrain] no PRETRAIN_CKPT; init from Kontext transformer + random delta heads"
fi
...
echo "Launching EditFlow Kontext residual (no alpha, no GAN, oss ${OSS_PROB} / pico ${PICO_PROB}): nproc=${NUM_GPUS} nfe=${NFE} shift=${SHIFT} load_from=${LOAD_FROM:-none} resume_from=${RESUME_FROM:-none} total_iters=${TOTAL_ITERS} run=${RUN_NAME} model=${KONTEXT_MODEL} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
