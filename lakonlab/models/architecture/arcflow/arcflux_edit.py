...
from .arc_output import ArcFlowEditModelOutput
from .arcflux import _ArcFluxTransformer2DModel


class _ArcFluxEditTransformer2DModel(_ArcFluxTransformer2DModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.proj_out_epsilon = nn.Linear(self.inner_dim, self.num_gaussians * self.out_channels)
    ...
    def init_weights(self):
        constant_init(self.proj_out_deltax.to_empty(device='cpu'), val=0)
        rand_noise = torch.randn(
            (self.num_gaussians * self.out_channels // self.logweights_channels)) * 0.1
        self.proj_out_deltax.bias.data.copy_(
...
class ArcFluxEditTransformer2DModel(_ArcFluxEditTransformer2DModel):
    ...
    def unpatchify(self, mp):
        if self.patch_size > 1:
            ...
            for key in ('deltax', 'epsilon'):
                mp[key] = mp[key].reshape(
    ...
    def forward(
        ...
        output['epsilon'] = output['epsilon'][:, :target_seq_len].permute(0, 2, 3, 1).reshape(
            bs, self.num_gaussians, self.out_channels, h, w)
