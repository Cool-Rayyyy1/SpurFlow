...
"""FLUX.2 Klein backbone + vanilla ArcFlow GM heads (means / logweights / loggamma).

Mirrors Kontext ``ArcFluxTransformer2DModel`` / Qwen ``ArcQwenEditArcFlowImageTransformer2DModel``
on FLUX.2-klein-base-9B editing: ``ArcFlowImitation`` + ``policy_type='ArcFlow'``.
Source latents are concatenated as ``image_latents`` (no uedit / alpha).
...
class ArcFlux2Transformer2DModel(Flux2Transformer2DModel):
    """Vanilla ArcFlow GM student for FLUX.2 Klein (means / logweights / loggamma)."""
    ...
    def __init__(
            ...
            *args,
            ...
            inherit_proj_out_means: bool = True,
            freeze: bool = False,
            freeze_exclude=(),
        ...
        self.inherit_proj_out_means = bool(inherit_proj_out_means)
        kwargs.pop('num_gaussians', None)
        kwargs.pop('logweights_channels', None)
        kwargs.pop('num_gammas', None)
        kwargs.pop('inherit_proj_out_means', None)
        ...
        self.proj_out_means = nn.Linear(inner, self.num_gaussians * out_channels)
        ...
        for head in (
                self.proj_out_means, self.proj_out_logweights, self.proj_out_loggamma):
        ...
        if dtype is not None:
            self.proj_out_means.to(device=device, dtype=dtype)
    ...
    def _init_arcflow_heads(self):
        inherited = False
        if self.inherit_proj_out_means and hasattr(self, 'proj_out'):
            materialize_meta_states(self.proj_out, device='cpu')
            weight = getattr(self.proj_out, 'weight', None)
            bias = getattr(self.proj_out, 'bias', None)
            if (weight is not None and not weight.is_meta
                    and weight.size(0) == self.out_channels):
                with torch.no_grad():
                    self.proj_out_means.weight.copy_(
                        weight[None].expand(self.num_gaussians, -1, -1).reshape(
                            self.num_gaussians * self.out_channels, -1))
                    if bias is not None and not bias.is_meta:
                        means_bias = bias[None].expand(
                            self.num_gaussians, -1).reshape(
                                self.num_gaussians * self.out_channels)
                        p2 = self.logweights_channels
                        rand_noise = torch.randn(
                            (self.num_gaussians * self.out_channels // p2),
                            dtype=means_bias.dtype,
                            device=means_bias.device) * 0.05
                        means_bias = means_bias + rand_noise[:, None].expand(
                            -1, p2).flatten()
                        self.proj_out_means.bias.copy_(means_bias)
                inherited = True
        if not inherited:
            constant_init(self.proj_out_means, val=0)
            rand_noise = torch.randn(
                (self.num_gaussians * self.out_channels // self.logweights_channels)
            ) * 0.1
            with torch.no_grad():
                self.proj_out_means.bias.copy_(
    ...
    def forward(
        ...
        out_means = self.proj_out_means(features).reshape(
        ...
        output_dict = dict(
            means=out_means.permute(0, 2, 3, 1).reshape(
