...
``train_flux_data.sh``, with source latents concatenated as ``image_latents``.
"""
...
class _ArcQwenEditArcFlowImageTransformer2DModel(QwenImageTransformer2DModel):
...
class ArcQwenEditArcFlowImageTransformer2DModel(_ArcQwenEditArcFlowImageTransformer2DModel):
    """Qwen-Image-Edit DiT + ArcFlow GM velocity heads for data-based PIID."""
    ...
    def init_weights(self, pretrained=None, pretrained_adapter=None):
        ...
        # Expand teacher proj_out -> mixture means (same as ArcQwen / ArcFlux).
    ...
    def forward(
        ...
        output = {
            'means': output.means[:, :target_seq_len].permute(0, 2, 3, 1).reshape(
