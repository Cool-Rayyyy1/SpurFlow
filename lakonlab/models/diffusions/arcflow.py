...
class ArcFlowImitationBase(GaussianFlow):
    ...
    def teacher_invert_to_noise(
            ...
            seq_len=None,
            eps=1e-4):
        """Flow ODE inversion: fixed teacher steps from (x_t, raw_t) to raw_t=1 (noise).

        Always runs exactly ``num_steps`` teacher forwards. Step size in raw_t is
        (1 - raw_t) / num_steps, so shorter paths use smaller steps automatically.
        """
        ...
        if not isinstance(raw_t, torch.Tensor):
            raw_t = torch.full((num_batches,), float(raw_t), dtype=torch.float32, device=device)
        else:
            raw_t = raw_t.reshape(num_batches).float().to(device)

        sigma = sigma_t.reshape(num_batches, *((ndim - 1) * [1])).float()
        t = sigma.flatten() * self.num_timesteps
        remaining = (1.0 - raw_t).clamp(min=eps)
        num_steps = max(int(num_steps), 1)

        for step_idx in range(num_steps):
            frac = float(step_idx + 1) / num_steps
            raw_t_next = (raw_t + remaining * frac).clamp(max=1.0)
            sigma_next = self.timestep_sampler.warp_t(raw_t_next, seq_len=seq_len).reshape(
                num_batches, *((ndim - 1) * [1]))
            u = teacher(return_u=True, x_t=x, t=t, **teacher_kwargs)
            x = x + u * (sigma_next - sigma)
            sigma = sigma_next
            t = sigma.flatten() * self.num_timesteps

        return x.to(ori_dtype)

    def teacher_invert_x0_to_noise(
            self,
            teacher,
            x_0,
            teacher_kwargs,
            ...
            seq_len=None,
            eps=1e-4):
        """Integrate teacher flow from x_0 (raw_t=0) to noise (raw_t=1) in fixed steps."""
        ...
        raw_t = torch.zeros(num_batches, dtype=torch.float32, device=device)
        sigma = self.timestep_sampler.warp_t(raw_t, seq_len=seq_len).reshape(
            num_batches, *((ndim - 1) * [1])).float()
        t = sigma.flatten() * self.num_timesteps

        for step_idx in range(num_steps):
            raw_t_next = torch.full(
                (num_batches,),
                float(step_idx + 1) / num_steps,
                dtype=torch.float32,
                device=device)
            raw_t_next = raw_t_next.clamp(min=eps, max=1.0)
            sigma_next = self.timestep_sampler.warp_t(raw_t_next, seq_len=seq_len).reshape(
                num_batches, *((ndim - 1) * [1]))
            u = teacher(return_u=True, x_t=x, t=t, **teacher_kwargs)
            x = x + u * (sigma_next - sigma)
...
class ArcFlowImitationDataFree(ArcFlowImitationBase):
    ...
    def forward_train(
        ...
        log_vars.update({
            ...
            f'loss_diffusion_step{step_id}': float(step_loss_diffusion.detach()),
        })
