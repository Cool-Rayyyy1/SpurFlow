#!/usr/bin/env bash
# EditFlow FLUX.2-klein-base Softsign-01 alpha + split-stage PIID + DINO GAN:
#   70% oss_edit banana-task pairs + 30% pico-banana-400k.
#   Student: ArcFlux2EditAlphaSoftsign01Transformer2DModel (proj_out_alpha).
#   OssEdit uses the same flux2 resize as ImageEdit (area-cap ~1MP, not Kontext buckets).
#
# Guidance:
...
#     bash train_flux2_klein_edit_fixedeps_alpha_softsign_alph_dino_gan_plus_oss.sh
#   FRESH=1 bash train_flux2_klein_edit_fixedeps_alpha_softsign_alph_dino_gan_plus_oss.sh
...
FRESH="${FRESH:-0}"
# Default: same no-GAN Softsign-01 alpha ckpt as the pico-only klein GAN recipe.
...
RUN_NAME="gmklein_base_uedit_fixedeps_alpha_softsign01_k16_${NFE}nfe_shift${SHIFT}_teachercfg${TEACHER_CFG}_oss${OSS_PROB}_pico${PICO_PROB}_split_stage_alph_dino_gan"
CONFIG="${PROJECT_DIR}/configs/flux2_klein/editflux2_klein_uedit_fixedeps_2nfe_k16_alpha_softsign_split_stage_alph_dino_gan_oss_pico.py"
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        LOAD_FROM=""
        echo "[resume] resuming klein-softsign-alph-dino-gan plus_oss run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
...
echo "Launching EditFlow FLUX.2-klein Softsign-01 alpha split-stage DINO GAN FSDP (oss ${OSS_PROB} / pico ${PICO_PROB}): nproc=${NUM_GPUS} nfe=${NFE} shift=${SHIFT} teacher_cfg=${TEACHER_CFG} student_cfg=1.0 total_iters=${TOTAL_ITERS} gan_weight=${STEP2_GAN_WEIGHT} gan_grad_step2_only=${GAN_GRAD_STEP2_ONLY} oss_root=${OSS_ROOT} pico_root=${PICO_ROOT} load_from=${LOAD_FROM:-none} resume_from=${RESUME_FROM:-none} run=${RUN_NAME} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
