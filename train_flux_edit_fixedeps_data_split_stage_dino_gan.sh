#!/usr/bin/env bash
# EditFlow fixed-eps split-stage rollout + step-2 TDM-style DINO feature GAN:
#   Loads pretrain ckpt (default iter_20000 from gmkontext_uedit_fixedeps_k16_2nfe_pico400k).
#   2-step rollout PIID from iter 0:
#     step-1 (t=1): PIID teacher loss
#     step-2 (student step-1 endpoint -> t=0): PIID + DINO feature GAN on rollout endpoint
#   GAN: DinoFeatureDiscriminator (global/local crops, layer-23 features, logistic loss).
#   Uses FSDP (8 GPUs default); VAE + DINO discriminator replicated per GPU.
#
#   bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh              # default: 8 GPUs
#   bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh 4
#   GPU_IDS=0,1 bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh 2
#
# GAN schedule / sample eval (env vars before bash):
#   SPLIT_STAGE_GAN_WARMUP_ITERS=3000 SPLIT_STAGE_GAN_RAMP_ITERS=500 \
#   SAMPLE_INTERVAL=500 bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh
#
# Pretrain / fresh:
#   PRETRAIN_CKPT=checkpoints/.../iter_20000.pth bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh
#   FRESH=1 bash train_flux_edit_fixedeps_data_split_stage_dino_gan.sh
...
SPLIT_STAGE_GAN_WEIGHT="${SPLIT_STAGE_GAN_WEIGHT:-0.001}"
DINO_GLOBAL_SIZE="${DINO_GLOBAL_SIZE:-224}"
DINO_LOCAL_SIZE="${DINO_LOCAL_SIZE:-224}"
DINO_FEATURE_LAYERS="${DINO_FEATURE_LAYERS:-23}"
# --------------------------------

RUN_NAME="gmkontext_uedit_fixedeps_k16_${NFE}nfe_pico400k_split_stage_dino_gan"
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        LOAD_FROM=""
        echo "[resume] resuming split-stage-dino-gan run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
...
echo "Launching EditFlow split-stage TDM-DINO GAN FSDP: nproc_per_node=${NUM_GPUS}  total_iters=${TOTAL_ITERS}  gan_warmup=${SPLIT_STAGE_GAN_WARMUP_ITERS}  gan_ramp=${SPLIT_STAGE_GAN_RAMP_ITERS}  gan_weight=${SPLIT_STAGE_GAN_WEIGHT}  sample_interval=${SAMPLE_INTERVAL}  x_ref_scale=${SPLIT_STAGE_STEP2_X_REF_SCALE}  global=${DINO_GLOBAL_SIZE}  local=${DINO_LOCAL_SIZE}  layer=${DINO_FEATURE_LAYERS}  load_from=${LOAD_FROM:-none}  resume_from=${RESUME_FROM:-none}  run=${RUN_NAME}  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" "${PROJECT_DIR}/train.py" \
    configs/kontext/editflux_uedit_fixedeps_2nfe_k16_data_split_stage_dino_gan.py \
