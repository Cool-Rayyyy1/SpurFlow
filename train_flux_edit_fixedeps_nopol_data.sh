#!/usr/bin/env bash
# EditFlow fixed path epsilon, epsilon NOT in policy (nopol variant):
#   Policy:  u = mixture(deltax_k; weights, gammas) + x_ref
#   Loss:    u_pred vs u_teacher = path_epsilon - x_0  (epsilon only here + path x_t)
#   Path:    x_t = (1 - sigma) * x_0 + sigma * path_epsilon  (fixed eps per trajectory)
#   Same 3-head student / data / NFE / LoRA as train_flux_edit_fixedeps_data.sh
#   deltax init: edit INHERIT_PROJ_OUT_DELTAX=false|true in user knobs below
#   Separate run name & checkpoints — does not touch fixedeps run.
#
#   bash train_flux_edit_fixedeps_nopol_data.sh              # default: 2 GPUs
#   bash train_flux_edit_fixedeps_nopol_data.sh 8
#   GPU_IDS=0,1,2,3,4,5,6,7 bash train_flux_edit_fixedeps_nopol_data.sh 8
#   TOTAL_ITERS=50000 bash train_flux_edit_fixedeps_nopol_data.sh
...
INHERIT_PROJ_OUT_DELTAX=false  # true: inherit FLUX proj_out -> deltax; false: random init deltax
# --------------------------------

RUN_NAME="gmkontext_uedit_fixedeps_nopol_k16_${NFE}nfe_pico400k"
...
echo "Launching EditFlow fixed-eps nopol DDP: nproc_per_node=${NUM_GPUS}  total_iters=${TOTAL_ITERS}  inherit_proj_out_deltax=${INHERIT_PROJ_OUT_DELTAX}  run=${RUN_NAME}  ckpts=checkpoints/${RUN_NAME}/  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" "${PROJECT_DIR}/train.py" \
    configs/kontext/editflux_uedit_fixedeps_nopol_2nfe_k16_data.py \
