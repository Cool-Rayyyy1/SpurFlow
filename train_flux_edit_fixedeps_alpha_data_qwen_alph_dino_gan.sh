#!/usr/bin/env bash
# EditFlow Qwen-Image-Edit alpha + split-stage PIID + source-cond DINO GAN.
# GAN recipe matches train_flux_edit_fixedeps_data_step2_alph_dino_gan.sh:
...
#
#   bash train_flux_edit_fixedeps_alpha_data_qwen_alph_dino_gan.sh              # default: 2 GPUs
#   bash train_flux_edit_fixedeps_alpha_data_qwen_alph_dino_gan.sh 8
#   GPU_IDS=0,1,2,3,4,5,6,7 bash train_flux_edit_fixedeps_alpha_data_qwen_alph_dino_gan.sh 8
#
# Pretrain / fresh (expects Qwen alpha ckpt with proj_out_alpha):
#   PRETRAIN_CKPT=checkpoints/.../iter_XXXX.pth bash train_flux_edit_fixedeps_alpha_data_qwen_alph_dino_gan.sh
#   FRESH=1 bash train_flux_edit_fixedeps_alpha_data_qwen_alph_dino_gan.sh
...
RUN_NAME="gmqwen_uedit_fixedeps_alpha_k16_${NFE}nfe_pico400k_split_stage_alph_dino_gan"
...
if [[ -n "${PRETRAIN_CKPT}" && -e "${PROJECT_DIR}/${PRETRAIN_CKPT}" ]]; then
    ...
    echo "[pretrain] loading Qwen alpha student weights from ${LOAD_FROM}"
elif [[ -n "${PRETRAIN_CKPT}" ]]; then
    echo "[pretrain] PRETRAIN_CKPT not found (${PRETRAIN_CKPT})." >&2
    if [[ "${ALLOW_NO_PRETRAIN:-0}" == "1" ]]; then
        echo "[pretrain] ALLOW_NO_PRETRAIN=1; starting without alpha pretrain (mask crop may fallback)." >&2
    else
        echo "[pretrain] Refuse to start without alpha weights. Set ALLOW_NO_PRETRAIN=1 to override." >&2
        exit 1
    fi
fi
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        LOAD_FROM=""
        echo "[resume] resuming qwen-alph-dino-gan run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
