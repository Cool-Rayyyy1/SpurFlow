#!/usr/bin/env bash
# EditFlow Qwen-Image-Edit Softsign-01 alpha + split-stage PIID + source-cond DINO GAN.
...
#   gan_weight=0.05, gan_grad_step2_only=true (Qwen default; both-NFE OOMs on 80GB)
...
#
#   bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan.sh              # default: 2 GPUs
#   bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan.sh 8
#   GPU_IDS=0,1,2,3,4,5,6,7 bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan.sh 8
#
# Pretrain / fresh (expects Softsign-01 alpha ckpt with proj_out_alpha):
#   PRETRAIN_CKPT=checkpoints/.../iter_XXXX.pth bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan.sh
#   FRESH=1 bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan.sh
...
CKPT_MUST_SAVE_INTERVAL="${CKPT_MUST_SAVE_INTERVAL:-500}"
...
RUN_NAME="gmqwen_uedit_fixedeps_alpha_softsign01_k16_${NFE}nfe_pico400k_split_stage_alph_dino_gan"
CONFIG="${PROJECT_DIR}/configs/qwen/editqwen_uedit_fixedeps_2nfe_k16_alpha_softsign_split_stage_alph_dino_gan.py"
...
# Reduce allocator fragmentation near the FSDP G-backward peak.
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        LOAD_FROM=""
        echo "[resume] resuming qwen-softsign-alph-dino-gan run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
...
echo "Launching EditFlow Qwen Softsign-01 alpha split-stage DINO GAN FSDP: nproc_per_node=${NUM_GPUS}  total_iters=${TOTAL_ITERS}  sample_interval=${SAMPLE_INTERVAL}  gan_weight=${STEP2_GAN_WEIGHT}  direct_delta_weight=${DIRECT_DELTA_WEIGHT}  gan_grad_step2_only=${GAN_GRAD_STEP2_ONLY}  condition_on_source=${CONDITION_ON_SOURCE}  p_disable_local=${P_DISABLE_LOCAL}  load_from=${LOAD_FROM:-none}  resume_from=${RESUME_FROM:-none}  run=${RUN_NAME}  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
