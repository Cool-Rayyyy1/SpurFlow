#!/usr/bin/env bash
# EditFlow Qwen-Image-Edit Softsign-01 alpha + split-stage PIID + DINO GAN:
...
#   OssEdit uses the same Qwen resize as ImageEdit (not Kontext buckets).
#
#   PRETRAIN_CKPT=checkpoints/.../iter_XXXX.pth \
#     bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan_plus_oss.sh
#   FRESH=1 bash train_flux_edit_fixedeps_alpha_softsign_data_qwen_alph_dino_gan_plus_oss.sh
...
RUN_NAME="gmqwen_uedit_fixedeps_alpha_softsign01_k16_${NFE}nfe_oss70_pico30_split_stage_alph_dino_gan"
CONFIG="${PROJECT_DIR}/configs/qwen/editqwen_uedit_fixedeps_2nfe_k16_alpha_softsign_split_stage_alph_dino_gan_oss_pico.py"
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        echo "[resume] resuming qwen-softsign-alph-dino-gan plus_oss run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
...
if [[ -z "${RESUME_FROM}" ]]; then
    if [[ -n "${PRETRAIN_CKPT}" ]]; then
        ...
        echo "[pretrain] PRETRAIN_CKPT is empty. Pass a Softsign-01 alpha ckpt, or set ALLOW_NO_PRETRAIN=1." >&2
        exit 1
    fi
fi
