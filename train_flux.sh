#!/usr/bin/env bash
# EditFlow: FLUX Kontext image-editing distillation on pico-banana-400k.
#
#   bash train_flux.sh              # default: 8 GPUs, CUDA 0-7
#   bash train_flux.sh 2            # override to 2 GPUs
#   TOTAL_ITERS=50000 bash train_flux.sh

set -euo pipefail

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Activate arcflow conda env (provides torchrun, PyTorch, deps).
# shellcheck source=/dev/null
source "${PROJECT_DIR}/setup_env.sh"

# Optional positional arg: bash train_flux.sh <num_gpus>
if [[ $# -ge 1 && "${1}" =~ ^[0-9]+$ ]]; then
    NUM_GPUS="${1}"
    shift
else
...
# ---------- user knobs ----------
NFE="${NFE:-2}"                         # distillation / inference steps (train_cfg.nfe == test_cfg.nfe)
CKPT_INTERVAL="${CKPT_INTERVAL:-500}"
CKPT_MUST_SAVE_INTERVAL="${CKPT_MUST_SAVE_INTERVAL:-1000}"
SAMPLE_INTERVAL="${SAMPLE_INTERVAL:-100}"
TOTAL_ITERS="${TOTAL_ITERS:-10000}"     # training iterations (not epochs)
EVAL="${EVAL:-1}"                       # 1=on, 0=off
...
GPU_IDS="${GPU_IDS:-0,1,2,3,4,5,6,7}"   # default: 8x A100 on one node
RUN_ID="${RUN_ID:-}"
RESUME_RUN_DIR="${RESUME_RUN_DIR:-}"
# --------------------------------

RUN_NAME="gmkontext_k16_${NFE}nfe_pico400k"
...
echo "Launching EditFlow DDP: nproc_per_node=${NUM_GPUS}  total_iters=${TOTAL_ITERS}  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" "${PROJECT_DIR}/train.py" \
    configs/kontext/editflux_2nfe_k16.py \
