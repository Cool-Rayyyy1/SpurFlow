#!/usr/bin/env bash
# ImgEdit-Bench for SimpleTuner FLUX.1-Kontext LoRA SFT (pico-banana).
#
# Defaults match tasting: steps=28, guidance=3.5, lora_scale=1.0
#
# Score only (already generated):
#   SCORE_ONLY=1 OPENAI_SCORING_MODEL=gpt-4o \
#     RUN_TAG=imgedit_kontext_sft_pico_lora_10000 \
#     bash evaluation/run_imgedit_kontext_sft_lora_eval.sh

set -euo pipefail

WORKSPACE_ROOT="${WORKSPACE_ROOT:-/mnt/afs_zhangyunzhe}"
EDITFLOW_DIR="${EDITFLOW_DIR:-${WORKSPACE_ROOT}/EditFlow}"
EVAL_DIR="${EVAL_DIR:-${EDITFLOW_DIR}/evaluation/imgedit_bench}"
DATA_ROOT="${DATA_ROOT:-${WORKSPACE_ROOT}/dataset/imgedit}"

CONDA_ROOT="${CONDA_ROOT:-${WORKSPACE_ROOT}/miniconda3}"
CONDA_ENV="${CONDA_ENV:-arcflow}"

export HF_ENDPOINT="${HF_ENDPOINT:-https://hf-mirror.com}"
export HF_HOME="${HF_HOME:-${WORKSPACE_ROOT}/.cache/huggingface}"
export KONTEXT_MODEL_PATH="${KONTEXT_MODEL_PATH:-${WORKSPACE_ROOT}/pretrained_models/FLUX.1-Kontext-dev}"
export IMGEDIT_BENCH_ROOT="${IMGEDIT_BENCH_ROOT:-${DATA_ROOT}/benchmark/Benchmark}"
export PYTHONPATH="${EDITFLOW_DIR}:${PYTHONPATH:-}"

export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
NUM_GPUS="${NUM_GPUS:-1}"
export NUM_GPUS

DEFAULT_LORA_DIR="${EDITFLOW_DIR}/checkpoints/my-edit-model-sft-checkpoint-8500/model/final/sft/checkpoint-8500"
LORA_DIR="${LORA_DIR:-${DEFAULT_LORA_DIR}}"

RUN_NAME="${RUN_NAME:-kontext_sft_pico_lora_8500}"
RUN_TAG="${RUN_TAG:-imgedit_${RUN_NAME}}"
LORA_STEPS="${LORA_STEPS:-28}"
LORA_GUIDANCE="${LORA_GUIDANCE:-3.5}"
LORA_SCALE="${LORA_SCALE:-1.0}"
export LORA_STEPS LORA_GUIDANCE

SUITE="${SUITE:-all}"
MAX_SAMPLES="${MAX_SAMPLES:-}"
GEN_ONLY="${GEN_ONLY:-0}"
SCORE_ONLY="${SCORE_ONLY:-0}"
CPU_OFFLOAD="${CPU_OFFLOAD:-0}"
NUM_PROCESSES="${NUM_PROCESSES:-16}"
FORCE_SCORE="${FORCE_SCORE:-0}"
export OPENAI_SCORING_MODEL="${OPENAI_SCORING_MODEL:-gpt-4o}"

RUN_OUTPUT_ROOT="${RUN_OUTPUT_ROOT:-${EVAL_DIR}/outputs/runs/${RUN_TAG}}"
MODEL_OUTPUT="${MODEL_OUTPUT:-${RUN_OUTPUT_ROOT}/lora}"
SCORES_DIR="${MODEL_OUTPUT}/scores"
SCORES_TXT="${MODEL_OUTPUT}/scores.txt"
RUN_CONFIG_TXT="${RUN_OUTPUT_ROOT}/run_config.txt"

write_run_config() {
  local status="${1:-started}"
  mkdir -p "${RUN_OUTPUT_ROOT}"
  {
    echo "ImgEdit-Bench Kontext SFT LoRA"
    echo "=============================="
    echo "Status:          ${status}"
    echo "Updated:         $(date '+%Y-%m-%d %H:%M:%S')"
    echo "RUN_TAG:         ${RUN_TAG}"
    echo "BASE_MODEL:      ${KONTEXT_MODEL_PATH}"
    echo "LORA_DIR:        ${LORA_DIR}"
    echo "LORA_STEPS:      ${LORA_STEPS}"
    echo "LORA_GUIDANCE:   ${LORA_GUIDANCE}"
    echo "LORA_SCALE:      ${LORA_SCALE}"
    echo "SUITE:           ${SUITE}"
    echo "MODEL_OUTPUT:    ${MODEL_OUTPUT}"
    echo "OPENAI_SCORING_MODEL: ${OPENAI_SCORING_MODEL}"
    echo "CUDA_VISIBLE_DEVICES: ${CUDA_VISIBLE_DEVICES}"
    echo "NUM_GPUS:        ${NUM_GPUS}"
    echo "MAX_SAMPLES:     ${MAX_SAMPLES:-all}"
  } > "${RUN_CONFIG_TXT}"
  echo "[config] ${RUN_CONFIG_TXT}"
}

run_score() {
  local -a cmd=(
    python "${EVAL_DIR}/run_imgedit_score.py"
    --role student
    --model_output "${MODEL_OUTPUT}"
    --scores_dir "${SCORES_DIR}"
    --scores_txt "${SCORES_TXT}"
    --bench_root "${IMGEDIT_BENCH_ROOT}"
    --annotations_dir "${EVAL_DIR}/annotations"
    --num_processes "${NUM_PROCESSES}"
  )
  if [[ "${FORCE_SCORE}" == "1" ]]; then
    cmd+=(--force)
  fi
  echo "Running: ${cmd[*]}"
  "${cmd[@]}"
}

# shellcheck source=/dev/null
source "${CONDA_ROOT}/etc/profile.d/conda.sh"
conda activate "${CONDA_ENV}"
# shellcheck source=/dev/null
source "${EDITFLOW_DIR}/setup_env.sh" 2>/dev/null || source "${EDITFLOW_DIR}/env.sh" 2>/dev/null || true
export HF_HUB_OFFLINE=0 TRANSFORMERS_OFFLINE=0 DIFFUSERS_OFFLINE=0

cd "${EDITFLOW_DIR}"

OPENAI_ENV="${EVAL_DIR}/openai.env"
if [[ -f "${OPENAI_ENV}" ]]; then
  # shellcheck source=/dev/null
  source "${OPENAI_ENV}"
fi

if [[ ! -d "${MODEL_OUTPUT}" ]]; then
  echo "ERROR: model output not found: ${MODEL_OUTPUT}" >&2
  echo "Generate first, or set RUN_TAG / MODEL_OUTPUT correctly." >&2
  exit 1
fi

write_run_config "started"

if [[ "${SCORE_ONLY}" != "1" && "${GEN_ONLY}" != "1" ]]; then
  echo "ERROR: generation path needs lora infer support; use SCORE_ONLY=1 for scoring," >&2
  echo "       or restore run_editflow_imgedit_infer.py with --role lora." >&2
  exit 1
fi

if [[ "${SCORE_ONLY}" != "1" ]]; then
  echo "ERROR: GEN path temporarily unavailable in this restored script. Use SCORE_ONLY=1." >&2
  exit 1
fi

if [[ "${GEN_ONLY}" != "1" ]]; then
  echo "[score] ImgEdit GPT (${OPENAI_SCORING_MODEL}) -> ${SCORES_TXT}"
  run_score
fi

write_run_config "completed"

echo ""
echo "Done."
echo "  run folder: ${RUN_OUTPUT_ROOT}"
echo "  outputs:    ${MODEL_OUTPUT}"
if [[ "${GEN_ONLY}" != "1" ]]; then
  echo "  scores:     ${SCORES_TXT}"
fi
