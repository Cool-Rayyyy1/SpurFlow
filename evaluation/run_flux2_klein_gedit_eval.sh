#!/usr/bin/env bash
# GEdit-v2 official distilled FLUX.2-klein-9B baseline, then GPT-4.1 VIEScore.
#
...
#
# Usage:
#   CUDA_VISIBLE_DEVICES=0 NUM_GPUS=1 bash evaluation/run_flux2_klein_gedit_eval.sh
#   GEN_ONLY=1 bash evaluation/run_flux2_klein_gedit_eval.sh
#   SCORE_ONLY=1 bash evaluation/run_flux2_klein_gedit_eval.sh
#   MAX_SAMPLES=8 GEN_ONLY=1 bash evaluation/run_flux2_klein_gedit_eval.sh
...
# Keep caller override; env.sh below pins klein-base and would otherwise win.
_USER_KLEIN_MODEL_PATH="${KLEIN_MODEL_PATH:-}"

# shellcheck source=/dev/null
source "${CONDA_ROOT}/etc/profile.d/conda.sh"
conda activate "${CONDA_ENV}"
# shellcheck source=/dev/null
source "${EDITFLOW_DIR}/env.sh" 2>/dev/null || source "${EDITFLOW_DIR}/setup_env.sh" 2>/dev/null || true

# Distilled 4-step Klein (NOT klein-base). env.sh must not override this.
export KLEIN_MODEL_PATH="${_USER_KLEIN_MODEL_PATH:-/mnt/afs_gaochengmin/checkpoints/FLUX.2-klein-9B}"
export KLEIN_STEPS="${KLEIN_STEPS:-4}"
export KLEIN_GUIDANCE="${KLEIN_GUIDANCE:-1.0}"
...
if [[ "${KLEIN_MODEL_PATH}" == *klein-base* ]]; then
  echo "ERROR: this script is the distilled FLUX.2-klein-9B 4-step baseline, not klein-base." >&2
  echo "  got: ${KLEIN_MODEL_PATH}" >&2
  echo "  expected: /mnt/afs_gaochengmin/checkpoints/FLUX.2-klein-9B" >&2
  exit 1
fi
...
write_run_config() {
  ...
  {
    echo "GEdit-v2 FLUX.2-klein-9B official distilled 4-step baseline"
    echo "=========================================================="
    echo "Status:              ${status}"
    echo "RUN_TAG:             ${RUN_TAG}"
    echo "MODEL:               ${KLEIN_MODEL_PATH}"
    echo "META_JSON:           ${META_JSON}"
    echo "STEPS:               ${KLEIN_STEPS}   # official distilled 4, not base 50"
...
if [[ "${SCORE_ONLY}" != "1" ]]; then
  ...
  echo "[gen] FLUX.2-klein-9B distilled GEdit-v2 (${KLEIN_STEPS} steps, cfg=${KLEIN_GUIDANCE})"
