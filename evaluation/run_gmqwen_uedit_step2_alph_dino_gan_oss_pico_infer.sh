#!/usr/bin/env bash
# ImgEdit-Bench (gen + gpt-4.1 score) for the Qwen Softsign-01 alpha
# step-2 DINO GAN student trained on oss30 / pico70.
#
# Training counterpart:
#   train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh
#   (NFE=2, OSS_PROB=0.3, student CFG=1.0, resize_mode=qwen)
#
# Must match train:
#   CONFIG  = editqwen_uedit_fixedeps_2nfe_k16_alpha_softsign_step2_alph_dino_gan_oss_pico.py
#   student = 2-NFE, guidance=1.0, STUDENT_RESIZE_MODE=qwen
#   teacher CFG=4.0 is train-only; do not pass 4.0 at student infer.
#
# Usage (1 GPU):
#   CUDA_VISIBLE_DEVICES=0 NUM_GPUS=1 \
#     bash evaluation/run_gmqwen_uedit_step2_alph_dino_gan_oss_pico_infer.sh
#
...
export RUN_NAME="${RUN_NAME:-gmqwen_uedit_fixedeps_alpha_softsign01_k16_2nfe_oss30_pico70_step2_alph_dino_gan}"
export CONFIG="${CONFIG:-${EDITFLOW_DIR}/configs/qwen/editqwen_uedit_fixedeps_2nfe_k16_alpha_softsign_step2_alph_dino_gan_oss_pico.py}"
export CKPT="${CKPT:-${EDITFLOW_DIR}/checkpoints/${RUN_NAME}/20260829_011114/iter_2400.pth}"
...
export SKIP_ALPHA_VIS="${SKIP_ALPHA_VIS:-0}"
...
write_run_config() {
  ...
  {
    echo "ImgEdit-Bench Qwen Softsign-01 Alpha step2 DINO GAN Student (+ alpha v6)"
    echo "======================================================================"
    echo "Status:          ${status}"
    ...
    echo "SKIP_ALPHA_VIS:  ${SKIP_ALPHA_VIS}"
...
echo "[gmqwen step2 alph_dino_gan oss30/pico70 / ImgEdit]"
echo "  CONFIG=${CONFIG}"
echo "  CKPT=${CKPT}"
echo "  RUN_TAG=${RUN_TAG}"
...
if [[ "${SCORE_ONLY}" != "1" ]]; then
  echo "[gen] Qwen Softsign-01 alpha student (${STUDENT_NFE} NFE, guidance=${STUDENT_GUIDANCE}, resize=${STUDENT_RESIZE_MODE}, skip_alpha_vis=${SKIP_ALPHA_VIS})"
  run_student_generation
fi
