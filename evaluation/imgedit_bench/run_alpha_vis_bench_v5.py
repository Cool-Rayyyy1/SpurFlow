#!/usr/bin/env python3
"""Alpha v5: per-case folder with src/edit/prompt + step1/step2 red-green maps.

Layout:
  <output_dir>/<category>/exampleK/
    prompt.txt
    src.png
    edit.png
...
Categories match ImgEdit-Bench basic suite (action/add/adjust/...).
Default: 2 seeded examples per category (same as training sample dumps).
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Alpha v5: category/example folders with step1+step2 red/green maps.")
    ...
    p.add_argument(
        ...
        type=int,
        default=int(os.environ.get("SAMPLES_PER_CATEGORY", "2")),
    )
    ...
    p.add_argument(
        ...
        help="Soft red/green tint strength for heatmap (src still visible).",
    )
    p.add_argument(
        "--overlay_blend",
        type=float,
        default=float(os.environ.get("OVERLAY_BLEND", "0.52")),
        help="Slightly stronger red/green tint for overlay.",
    )
    return p.parse_args()
...
def v5_case_dir(output_dir: Path, category: str, example_name: str) -> Path:
    return output_dir / category / example_name


def v5_output_paths(case_dir: Path) -> Dict[str, Path]:
    return {
...
def process_samples(
    ...
    if num_steps != 2:
        raise ValueError(f"Alpha v5 expects nfe=2 so both steps are visualized, got {num_steps}.")

    manifest: Dict[str, Dict] = {}
    for idx, sample in enumerate(tqdm(samples, desc=desc)):
        case_dir = v5_case_dir(output_dir, sample["category"], sample["example_name"])
        paths = v5_output_paths(case_dir)
...
def main() -> None:
    ...
    # Reuse round-robin splitter: wrap as (task_key, sample) tuples.
