#!/usr/bin/env bash
# ImgEdit-Bench for train_flux_data.sh (vanilla ArcFlow, no alpha).
# All local paths under /mnt/afs_gaochengmin.
#
# Default ckpt: checkpoints/20260817_110510/latest.pth
#
# Usage:
#   bash evaluation/run_gmkontext_data_imgedit_eval.sh
#   GEN_ONLY=1 bash evaluation/run_gmkontext_data_imgedit_eval.sh
#   SUITE=basic MAX_SAMPLES=8 bash evaluation/run_gmkontext_data_imgedit_eval.sh
...
echo "[gmkontext_data / ImgEdit] CONFIG=${CONFIG}"
echo "[gmkontext_data / ImgEdit] CKPT=${CKPT}"
echo "[gmkontext_data / ImgEdit] RUN_TAG=${RUN_TAG}"
echo "[gmkontext_data / ImgEdit] NFE=${STUDENT_NFE}  guidance=${STUDENT_GUIDANCE}  resize=${STUDENT_RESIZE_MODE}"
