#!/usr/bin/env bash
# 2nd GPU on this machine (physical GPU index 1): latent-space x0_hat vs x_src and x_edit
set -euo pipefail
...
python tools/kontext_x0_ref_diff.py \
  ...
  --guidance-scale 2.5 \
  --seed 42 \
  --output-json "${OUT_DIR}/latent_metrics.json" \
  "${EXTRA[@]}" \
  "$@" \
  2>&1 | tee "${OUT_DIR}/latent_run.log"

echo "Done. Log: ${OUT_DIR}/latent_run.log"
