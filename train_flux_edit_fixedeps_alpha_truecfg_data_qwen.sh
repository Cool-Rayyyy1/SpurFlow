#!/usr/bin/env bash
# EditFlow fixed-eps + Softsign-01 alpha on Qwen, with student true-CFG aligned to teacher:
#   alpha = 0.5 * (x / (1 + abs(x)) + 1)
#   teacher / student both run cond + uncond
#   same CFG scale=4.0 + Qwen norm rescale
#   loss matches CFG-combined velocities
#
# Does NOT touch gmqwen_uedit_fixedeps_alpha_k16_* (sigmoid / cond-only student) runs.
#
#   bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh              # default: 2 GPUs
#   GPU_IDS=0 bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh 1  # single GPU
#   bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh 8
#   TOTAL_ITERS=50000 bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh
#
# Resume:
#   Auto-detects the most recent run under
#   checkpoints/gmqwen_uedit_fixedeps_alpha_softsign01_truecfg_k16_*.
#   RESUME_RUN_DIR=<run_id> bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh
#   FRESH=1 bash train_flux_edit_fixedeps_alpha_truecfg_data_qwen.sh
...
RUN_NAME="gmqwen_uedit_fixedeps_alpha_softsign01_truecfg_k16_${NFE}nfe_pico400k"
...
echo "Launching EditFlow Qwen uedit fixed-eps+Softsign01-alpha true-CFG FSDP: nproc_per_node=${NUM_GPUS}  total_iters=${TOTAL_ITERS}  sample_interval=${SAMPLE_INTERVAL}  samples_per_category=${SAMPLES_PER_CATEGORY}  run=${RUN_NAME}  ckpts=checkpoints/${RUN_NAME}/  model=${QWEN_MODEL}  CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

torchrun --nnodes=1 --nproc_per_node="${NUM_GPUS}" "${PROJECT_DIR}/train.py" \
    configs/qwen/editqwen_uedit_fixedeps_2nfe_k16_alpha_truecfg_data.py \
