#!/usr/bin/env bash
# EditFlow Qwen Softsign-01 alpha + standard PIID + independent step-2 2-NFE
# rollout DINO GAN (Kontext step2_alph_dino_gan recipe on Qwen):
...
#   Student: ArcQwenEditAlphaSoftsign01ImageTransformer2DModel (proj_out_alpha)
#   GAN: DinoAlphaMaskFeatureDiscriminator
#     global + random local + alpha-mask local crops
#     condition_on_source: cat([DINO(ref), DINO(target)])
#   Data default: 30% oss_edit + 70% pico (matches Softsign-01 pretrain mix)
#   Sample eval: ImgEdit + GEdit-v2
#
#   bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh              # default: 8 GPUs
#   bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh 4
#   GPU_IDS=0,1,2,3 bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh 4
#
# Pretrain (Softsign-01 alpha ckpt required by default):
#   PRETRAIN_CKPT=checkpoints/.../iter_XXXX.pth bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh
#   FRESH=1 bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh
#   ALLOW_NO_PRETRAIN=1 bash train_flux_edit_fixedeps_data_qwen_step2_dino_gan.sh
...
CKPT_MUST_SAVE_INTERVAL="${CKPT_MUST_SAVE_INTERVAL:-200}"
SAMPLE_INTERVAL="${SAMPLE_INTERVAL:-200}"
...
FRESH="${FRESH:-0}"
# Softsign-01 alpha student (oss30/pico70) iter_17500.
PRETRAIN_CKPT="${PRETRAIN_CKPT:-checkpoints/gmqwen_uedit_fixedeps_alpha_softsign01_k16_2nfe_oss30_pico70/20260819_150749/iter_17500.pth}"
...
RUN_NAME="gmqwen_uedit_fixedeps_alpha_softsign01_k16_${NFE}nfe_oss${oss_pct}_pico${pico_pct}_step2_alph_dino_gan"
CONFIG="${PROJECT_DIR}/configs/qwen/editqwen_uedit_fixedeps_2nfe_k16_alpha_softsign_step2_alph_dino_gan_oss_pico.py"
...
if [[ "${FRESH}" != "1" ]]; then
    ...
    if [[ -n "${RESUME_RUN_ID}" && -e "${PROJECT_DIR}/${CKPT_BASE}/${RESUME_RUN_ID}/latest.pth" ]]; then
        ...
        LOAD_FROM=""
        echo "[resume] resuming qwen-softsign-step2-alph-dino-gan run_id=${RESUME_RUN_ID} from ${RESUME_FROM}"
    fi
fi
...
# Sample dump + AFS I/O can pause ranks for >10min; default NCCL watchdog (600s) then kills the job.
export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC="${TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC:-1800}"
export NCCL_TIMEOUT="${NCCL_TIMEOUT:-1800}"

echo "Launching EditFlow Qwen Softsign-01 STEP2 alph DINO GAN FSDP (oss ${OSS_PROB} / pico ${PICO_PROB}): nproc=${NUM_GPUS} nfe=${NFE} total_iters=${TOTAL_ITERS} gan_weight=${STEP2_GAN_WEIGHT} gan_grad_step2_only=${GAN_GRAD_STEP2_ONLY} condition_on_source=${CONDITION_ON_SOURCE} p_disable_local=${P_DISABLE_LOCAL} load_from=${LOAD_FROM:-none} resume_from=${RESUME_FROM:-none} run=${RUN_NAME} CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"
