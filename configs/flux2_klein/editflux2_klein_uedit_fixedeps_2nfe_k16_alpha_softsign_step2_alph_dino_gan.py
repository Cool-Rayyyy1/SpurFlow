...
# `train_flux2_klein_edit_fixedeps_alpha_softsign_step2_alph_dino_gan*.sh`
# FLUX.2-klein-base Softsign-01 alpha + standard PIID + independent step-2
# 2-NFE rollout for source-cond DINO GAN (Kontext step2_alph_dino_gan recipe).
#
# Softsign-01 (same as klein softsign baseline):
...
#
# GAN recipe matches Kontext step2_alph_dino_gan:
#   DinoAlphaMaskFeatureDiscriminator, condition_on_source,
#   cat([DINO(ref), DINO(target)]), alpha-mask local crop from rolled-out
#   step-2 alpha, gan_weight=0.05, gan_grad_step2_only=False.
#
...
model = dict(
    type='LatentDiffusionFlux2KleinImageEditStep2AlphaDinoGAN',
    vae=dict(
...
train_cfg = dict(
    ...
    # Shared GAN weight / schedule keys (name kept for helper compatibility).
