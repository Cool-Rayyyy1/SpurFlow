...
# `train_flux_edit_fixedeps_data_qwen_split_stage_dino_gan.sh`
# Qwen-Image-Edit fixed-eps split-stage PIID + DINO feature GAN (no alpha).
# Same unroll sharing as alph_dino_gan:
#   ArcFlowEditImitationSplitStageGAN — step1/step2 PIID in one 2-NFE chain
#   GAN endpoint reuses step-2 integrate-to-t=0 (no extra independent rollout)
#   DinoFeatureDiscriminator: random global/local crops (not alpha-mask)
#   condition_on_source=True: cat([DINO(ref), DINO(target)])
#   gan_grad_step2_only=False (GAN grads through both NFE steps, like alph)
name = 'gmqwen_uedit_fixedeps_k16_2nfe_pico400k_split_stage_dino_gan'
...
model = dict(
    type='LatentDiffusionQwenImageEditSplitStageDinoFeatureGAN',
    vae=dict(
...
train_cfg = dict(
    ...
    # Match alph_dino_gan: keep GAN grads through both NFE steps.
