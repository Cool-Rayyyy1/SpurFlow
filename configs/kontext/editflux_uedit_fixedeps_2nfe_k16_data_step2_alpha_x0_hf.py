...
# `train_flux_edit_fixedeps_data_step2_alpha_x0_hf.sh`
# Alpha PIID + latent edit-HF (NO GAN, NO RGB x0):
#   L = L_PIID + λ_hf ||H(Δ_S) - H(Δ_GT)||_1
# Δ on unpatchified latents, alpha-mask local crop; H(z)=z-GaussianBlur(z).
name = 'gmkontext_uedit_fixedeps_alpha_k16_2nfe_pico400k_step2_latent_hf'
...
model = dict(
    type='LatentDiffusionImageEditStep2AlphaX0HF',
    vae=dict(
        type='PretrainedVAE',
...
train_cfg = dict(
    ...
    # Endpoint HF backprop through final NFE only (cheaper / stabler).
    gan_grad_step2_only=True,
    # RGB x0 L1 removed; latent HF on alpha-guided local crop only.
    x0_loss_weight=0.0,
    hf_loss_weight=0.3,
    latent_crop_size=64,
    hf_blur_sigma=1.0,
    alpha_crop_p_disable_local=0.0,
    ...
    alpha_crop_local_scale=(0.08, 0.35),
    alpha_crop_aspect_ratio=(0.75, 1.3333333333),
    num_decay_iters=0,
    window_substeps=3,
    gm_dropout=0.1,
