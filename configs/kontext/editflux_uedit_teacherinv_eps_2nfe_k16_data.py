...
# `train_flux_edit_teacherinv_eps_data.sh` -> gmkontext_uedit_teacherinv_eps_k16_2nfe_pico400k
# 4 heads: deltax / logweights / loggamma / epsilon
# Train: x_t from random noise; epsilon head predicts teacher-inverted path noise;
#        student_u = pred_epsilon - x_ref - pred_delta; + MSE(pred_epsilon, teacher_inv)
# Infer: x_t from random noise; pred_epsilon from epsilon head at t=1 (fixed trajectory)
name = 'gmkontext_uedit_teacherinv_eps_k16_2nfe_pico400k'
...
model = dict(
    ...
    diffusion=dict(
        type='ArcFlowEditTeacherInvEpsImitation',
        policy_type='ArcFlowEditNewEps',
        denoising=dict(
            type='ArcFluxEditNewEpsTransformer2DModel',
            patch_size=2,
            freeze=True,
            freeze_exclude=[
                'proj_out_deltax',
