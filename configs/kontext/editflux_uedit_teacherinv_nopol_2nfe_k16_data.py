...
train_cfg = dict(
    use_edited_x0=True,
    use_uedit=True,
    teacher_invert_epsilon=True,
    teacher_invert_steps=10,
    # Build x_t from fresh random noise (no target leakage); the residual
    # velocity still uses the teacher-inverted path_epsilon.
    xt_input_noise='random',
...
# FLUX.1 Kontext-style preprocessing for this version only: resize to the
# closest preferred-resolution bucket (no center crop). Variable per-sample
# resolution => samples_per_gpu must be 1 (cannot stack different shapes).
data = dict(
    workers_per_gpu=1,
