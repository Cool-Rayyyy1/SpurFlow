...
# `train_flux_edit_fixedeps_data.sh` -> gmkontext_uedit_fixedeps_k16_2nfe_pico400k
# Residual parameterization: pred_delta ~ x0_tgt - x_ref,
# student_u = path_epsilon - x_ref - pred_delta, teacher_u = path_epsilon - x0_tgt.
name = 'gmkontext_uedit_fixedeps_k16_2nfe_pico400k'
...
# FLUX.1 Kontext-style preprocessing: choose the closest preferred-resolution
# bucket from the source aspect ratio and plain-resize both source/edited images
# to that bucket (no center crop). Variable per-sample resolution requires
# samples_per_gpu=1 unless a resolution-bucketed sampler is used.
data = dict(
    workers_per_gpu=1,
